...
const osMessageQueueAttr_t Queue1Attributes = {
		.name = "Queue1"
};

osThreadId_t bootCheckTaskHandle;
const osThreadAttr_t bootCheckTask_attributes = {
  .name = "bootCheckTask",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 128 * 4
};
...
int main(void)
{
  ...
  /* add queues, ... */
  Queue1Handle = osMessageQueueNew(8, sizeof(uint8_t), &Queue1Attributes);
	 ...
	 uint32_t pressStartTime = 0;
	    ...
	    for (;;)
	    {
	        ...
	        if (pinState == GPIO_PIN_SET)
	        {
	            ...
	            else if (HAL_GetTick() - pressStartTime >= 300)
	            {
	            	HAL_GPIO_TogglePin(GPIOG, GPIO_PIN_13);
	            	uint8_t data = 'C';
	                osMessageQueuePut(Queue1Handle, &data, 0 , 10);
